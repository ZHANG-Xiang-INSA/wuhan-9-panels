...
const CLIP_COL = c => c === 'RC-50' ? '#2f6ea8' : '#b4491f';
...
const T = {
  zh: {
    ...
    th_used: '用于', th_total: '合计',
    note_exact: (b, c) => `实际用量：砖片 ${b} 片，卡扣 ${c} 个。`,
  ...
  en: {
    ...
    th_used: 'Used on', th_total: 'Total',
    note_exact: (b, c) => `As built: ${b} slips, ${c} clips.`,
...
function summary() {
  ...
  $('#sumclips').innerHTML = S.clips.map(e => `
    <tr>
      <td class="tid">${e.code}</td>
      <td class="kind">${lang === 'zh' ? e.zh : e.en}</td>
